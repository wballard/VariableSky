This is the client library, focused on a streamy socket interface.
...
## reconnect
Websocket stream reconnector. This does an exponential backoff that resets
on a successful connect, attempting to reconnect on any error or end that isn't
an explict close.
...
as well as the browser via [browserify]. You can `new` this.
...
This is what it looks like, http->ws, https->wss.
...
inbound path when the server returns the complete message, using `__id__` as
a correlation key.
...
Make sure all tracked links are hooked up. This is part of the automatic state
recovery that works with automatic reconnection.
...
if you hooked angular into a 'namespace' rather than just on `window`. And
notice angularjs isn't bundled in, behold the power of JavaScript.
...
Either the sky value or a default. This doesn't trigger a save back to the sky,
it is just a local client default. And if you don't specify a default, it just
stays undefined. JavaScript magic.
...
Push into angular scope land. This will trigger binding. I know the exitcount
thing looks fishy, but remember the undelying variable is updated as it is a
object on the blackboard also pointed to by angular.
...
* Save a diff, save a life (TM) -- well, at least some bytes
